...
import { damageTarget, nearestEnemyUnit } from '../targeting.js';
import type { GameState } from '../state.js';
import { powerBalance } from './production.js';

/**
 * Auto-firing base defenses (Tesla coil, pillbox). Classic rule: defenses go
 * OFFLINE while the owner has a power deficit.
 */
export function defenseSystem(state: GameState): void {
  ...
  for (const building of state.buildings) {
    ...
    const target = nearestEnemyUnit(state, building.owner, building.x, building.y, weapon.rangeSq);
    if (!target) continue;
