...
import { RESOURCE_NONE, cellIndex, cellsAroundRect, inBounds, isPassableTerrain } from '../map.js';
...
function spawnCrates(state: GameState): void {
  ...
  for (let attempt = 0; attempt < SPAWN_ATTEMPTS; attempt++) {
    ...
    const kind = roll <= 1 ? 'MONEY' : roll === 2 ? 'HEAL' : roll === 3 ? 'REVEAL' : 'UNIT';
...
function applyCrate(state: GameState, crate: Crate, ownerId: number): void {
  ...
  switch (crate.kind) {
    case 'MONEY':
      ...
      return;
    case 'HEAL': {
      // The collector plus every own unit near the crate gets patched up.
